...
class PlatformFFI {
  ...
  void _startListenEvent(IpmrmtImpl ipmrmtImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = ipmrmtImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
